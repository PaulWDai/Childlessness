...
clear
gen country = .
save "$data/nochild_urban_liefecycle.dta",replace
...
gen obs = 1
qui capture collapse (mean) nochild (rawsum) obs [aw=perwt], by(country year sample age urban)
qui append using "$data/nochild_urban_liefecycle.dta",nolabel
qui save "$data/nochild_urban_liefecycle.dta",replace
}


use "$data/nochild_urban_lifecycle.dta",clear
...
keep country year sample nochild age urban obs gdppc lngdp
save "$data/nochild_urban_lifecycle_merge.dta",replace
