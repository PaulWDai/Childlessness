...
* Author: Paul W Dai
* Update: Aug 21, 2021
* Purpose: Childlessness Rate by Education and Logged GDP per capita
********************************************************************************
...
clear
gen country = .
save "$data/nochild_ever_married_1554.dta",replace
...
qui capture gen obs = 1
qui capture collapse (mean) nochild (rawsum) count = obs (sum) weight = obs [aw=perwt], by(country year sample ever_married)
qui append using "$data/nochild_ever_married_1554.dta",nolabel
qui save "$data/nochild_ever_married_1554.dta",replace
}


use "$data/nochild_ever_married_1554.dta",clear
...
keep country year countrycode sample nochild ever_married gdppc lngdp count weight
bysort country year : egen total = sum(weight)
replace weight = weight/total
drop total

save "$data/nochild_ever_married_merge_1554.dta",replace
