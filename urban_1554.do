...
clear
gen country = .
save "$data/nochild_urban_1554.dta",replace
...
qui capture collapse (mean) nochild (rawsum) count = obs (sum) weight = obs [aw=perwt], by(country year sample urban)
qui append using "$data/nochild_urban_1554.dta",nolabel
qui save "$data/nochild_urban_1554.dta",replace
}

use "$data/nochild_urban_1554.dta",clear
...
keep country year countrycode sample nochild urban gdppc lngdp count weight
bysort country year : egen total = sum(weight)
replace weight = weight/total
drop total

save "$data/nochild_urban_merge_1554.dta",replace
