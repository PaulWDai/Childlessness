...
clear
gen country = .
save "$data/nochild_education_4554.dta",replace
...
// age
qui capture keep if age>=45 & age<=54 // prime aged
// education
...
qui capture collapse (mean) nochild (rawsum) obs[aw=perwt], by(country year sample edattain)
qui append using "$data/nochild_education_4554.dta",nolabel
qui save "$data/nochild_education_4554.dta",replace
}


use "$data/nochild_education_4554.dta",clear
...
keep country year countrycode sample nochild edattain obs gdppc lngdp


save "$data/nochild_education_merge_4554.dta",replace
