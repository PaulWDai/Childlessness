...
clear
gen country = .
save "$data/nochild_urban_4554.dta",replace
...
qui capture collapse (mean) nochild (rawsum) obs [aw=perwt], by(country year sample urban)
qui append using "$data/nochild_urban_4554.dta",nolabel
qui save "$data/nochild_urban_4554.dta",replace
}


use "$data/nochild_urban_4554.dta",clear
...
keep country year countrycode sample nochild urban obs gdppc lngdp

save "$data/nochild_urban_merge_4554.dta",replace
