...
clear
gen country = .
save "$data/nochild_ever_married_lifecycle.dta",replace
...
gen obs = 1
qui capture collapse (mean) nochild (rawsum) obs [aw=perwt], by(country year sample age ever_married)
qui append using "$data/nochild_ever_married_lifecycle.dta",nolabel
qui save "$data/nochild_ever_married_lifecycle.dta",replace
}


use "$data/nochild_ever_married_lifecycle.dta",clear
...
keep country year sample nochild age ever_married obs gdppc lngdp

save "$data/nochild_ever_married_lifecycle_merge.dta",replace
