...
clear
gen country = .
save "$data/nochild_employment_lifecycle.dta",replace
...
gen obs = 1
qui capture collapse (mean) nochild (rawsum) obs [aw=perwt], by(country year sample age empstat)
qui append using "$data/nochild_employment_lifecycle.dta",nolabel
qui save "$data/nochild_employment_lifecycle.dta",replace
}


use "$data/nochild_employment_lifecycle.dta",clear
...
keep country year sample nochild age empstat obs gdppc lngdp

save "$data/nochild_employment_lifecycle_merge.dta",replace
